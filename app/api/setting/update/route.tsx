...
export async function POST(request: Request) {
    ...
    //Update
    try{
        const results = await execQuery("UPDATE `setting` SET `Logo` = '"+ (req.Logo ?? "") + "', `LogoBase64` = '"+(req.LogoBase64 ?? "")+"', `Title` = '"+(req.Title ?? "")+"', `Des`='"+(req.Des ?? "")+"', `Tel`='"+(req.Tel ?? "")+"', `Fax`='"+(req.Fax ?? "")+"', `Email`='"+(req.Email ?? "")+"'  WHERE `Id` = "+req.Id);
